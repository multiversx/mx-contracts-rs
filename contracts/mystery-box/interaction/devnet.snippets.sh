...
deployMysteryBoxSC() {
    mxpy --verbose contract deploy --recall-nonce \
        ...
        --arguments ${MB_TOKEN} ${MB_COOLDOWN_EPOCHS_NO} \
        --send || return
}
...
setupMysteryBox() {
    mxpy --verbose contract call ${CONTRACT_ADDRESS} --recall-nonce \
        ...
        --arguments $XP_TYPE $XP_REWARD_TOKEN $XP_VALUE $XP_CHANCE $DEFAULT_GLOBAL_COOLDOWN \
        $MB_TYPE $MB_TOKEN $MB_VALUE $MB_CHANCE $DEFAULT_GLOBAL_COOLDOWN \
        $PERCENT_TYPE $PERCENT_TOKEN $PERCENT_VALUE $PERCENT_CHANCE $DEFAULT_GLOBAL_COOLDOWN \
        $FV_TYPE $FV_TOKEN $FV_VALUE $FV_CHANCE $FV_GLOBAL_COOLDOWN \
        --send || return
}
...
openMysteryBox() {
    ...
    mxpy --verbose contract call $user_address --recall-nonce \
        ...
        --arguments $MB_TOKEN $1 $MB_AMOUNT $CONTRACT_ADDRESS $method_name \
        --send || return
}
